...
class Model {
    ...
    async save(fields) {
        ...
        const query = `INSERT INTO ${this.name} (${cols.join(",")}) VALUES(${values.map(value => `'${value}'`).join(",")})`
        const response = await this.run(query)
        return response
    }
    ...
    async findByIdAndUpdate(id,fields){
        ...
        const query = `UPDATE ${this.name} SET ${entries.map(([col,value])=>`${col}='${value}'`).join(",")} WHERE id=${parseInt(id)};`
        const response = await this.run(query);
        return response;
    }
